...
var settingPage = {
    ...
    init: function () {
        ...
        //user messages
        $.M.get({
            method:"/message/notice.jhtml?memberId=" + settingPage.userId,
        ...
        $.M.get({
            ...
            success: function (data) {
                if (data.returnValue != "") {
                    ...
                    var messages = "";
                    for (var i = 0; i < messagesData.length; i++) {
                        messages = messages + "<div class='message_record_div'><div class='message_del' onclick='settingPage.messageRemove(" + messagesData[i].id + ");'><img src='../css/images/my/icon_delete.png' alt='' /></div>"
    ...
    messageRemove: function (messageId) {
        $.M.post({
            ...
            success: function (data) {
                if (data.success) {
                    settingPage.showMessages("操作成功");
                    settingPage.messageEdit();
                } else {
                    settingPage.showMessages("操作失败，失败原因：" + data.errorReason);
                }
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
            }
...
$(document).bind("pageshow", function (e) {
    ...
    if ("setting_page" == id) {
        ...
        if (!settingPage.scroll) {
            settingPage.scroll = new iScroll("underpart_div_scroll", {
                useTransition: true,
...
$(function () {
    ...
    $("#aboutApp_btn").click(function () {
        window.location.href = "aboutApp.html";
    });
    $("#logout_btn").click(function () {
        $.M.post({
            method: "/auth/loginOut.jhtml?memberId=" + settingPage.userId,
            success: function (data) {
                if (data.success) {
                    settingPage.showMessages("已登出");
                    pageLocalStorage.removeItem("userid");
