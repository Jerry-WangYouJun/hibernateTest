(function(a) {
	...
	a(document).bind("pagebeforecreate",
	function(g) {
	...
	function g(b, f) {
		...
		function l(a) {
			...
			d = 1,
			h;
			for (h in ca[a]) 0 == d % 20 && (b += '</div><div class="dw-bf">'),
			b += '<div class="dw-li dw-v" data-val="' + h + '" style="height:' + K + "px;line-height:" + K + 'px;"><div class="dw-i">' + ca[a][h] + "</div></div>",
			d++;
			return b + "</div>"
		}
		function g(b) {
		...
		function da(b, h, d, c, e) { ! 1 !== M("validate", [o, h, b]) && (a(".dw-ul", o).each(function(d) {
				var r = a(this),
				...
				if (!f.hasClass("dw-v")) {
					for (var o = f,
					w = 0,
					U = 0; 0 <= j - w && !o.hasClass("dw-v");) w++,
					o = z.eq(j - w);
					for (; j + U < l && !f.hasClass("dw-v");) U++,
					f = z.eq(j + U); (U < w && U && 2 !== c || !w || 0 > j - w || 1 == c) && f.hasClass("dw-v") ? j += U: (f = o, j -= w)
				}
				if (!f.hasClass("dw-sel") || g) i.temp[d] = f.attr("data-val"),
				a(".dw-sel", r).removeClass("dw-sel"),
				f.addClass("dw-sel"),
				i.scroll(r, d, j, g ? b: 0.1, g ? e: void 0)
			}), i.change(d))
		}
		function ga(b) {
		...
		i.show = function(b) {
			...
			for (var w = '<div class="dw-trans ' + h.theme + " dw-" + h.display + '">' + ("inline" == h.display ? '<div class="dw dwbg dwi"><div class="dwwr">': '<div class="dw-persp"><div class="dwo"></div><div class="dw dwbg ' + f + '"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="dwwr">' + (h.headerText ? '<div class="dwv"></div>': "")), b = 0; b < h.wheels.length; b++) {
				...
				for (e in h.wheels[b]) ca[c] = h.wheels[b][e],
				w += '<td><div class="dwwl dwrc dwwl' + c + '">' + ("scroller" != h.mode ? '<div class="dwwb dwwbp" style="height:' + K + "px;line-height:" + K + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + K + "px;line-height:" + K + 'px;"><span>&ndash;</span></div>': "") + '<div class="dwl">' + e + '</div><div class="dww" style="height:' + h.rows * K + "px;min-width:" + h.width + 'px;"><div class="dw-ul">',
				w += l(c),
				w += '</div><div class="dwwo"></div></div><div class="dwwol"></div></div></td>',
				c++;
				w += "</tr></table></div></div>"
			}
			...
			X = !0;
			ja.init(o, i);
			"inline" != h.display && (i.tap(a(".dwb-s span", o),
			function() {
				if (i.hide(false, "set") !== false) {
					i.setValue(false, true);
					M("onSelect", [i.val])
				}
			}), i.tap(a(".dwb-c span", o),
			function() {
				i.cancel()
			...
			o.delegate(".dwwl", "DOMMouseScroll mousewheel",
			function(b) {
				if (!j(this)) {
					b.preventDefault();
		...
		i.hide = function(b, c) {
			...
			350)) : (o.remove(), o = null), X = !1, ka = {},
			a(window).unbind(".dw"))
		};
		i.cancel = function() { ! 1 !== i.hide(!1, "cancel") && M("onCancel", [i.val])
		};
		i.init = function(b) {
			ja = C({
				defaults: {},
			...
			i.settings = h;
			B.unbind(".dw");
			if (b = ia.presets[h.preset]) ea = b.call(P, i),
			C(h, ea, f),
			C(fa, ea.methods);
			...
			X && i.hide();
			"inline" == h.display ? i.show() : (u(), Z && h.showOnFocus && (B.data("dwro", P.readOnly), P.readOnly = !0, B.bind("focus.dw",
			function() {
				i.show()
			})))
		};
	...
	function() {
		if (d) {
			...
			if (!g && !la) {
				...
				n = a(".dw-li", q).eq(l);
				g = ba; ! 1 !== x.trigger("onValueTap", [n]) ? b = l: g = !0;
	...
	y = function(F) {
		...
		if (q.match(/date/i)) {
			...
			for (e = 0; 3 > e; e++) if (e == l.y) {
				...
				m++;
				f[c.monthText] = {};
				for (d = 0; 12 > d; d++) L = H.replace(/[dy]/gi, "").replace(/mm/, 9 > d ? "0" + (d + 1) : d + 1).replace(/m/, d + 1),
				f[c.monthText][d] = L.match(/MM/) ? L.replace(/MM/, '<span class="dw-mon">' + c.monthNames[d] + "</span>") : L.replace(/M/, '<span class="dw-mon">' + c.monthNamesShort[d] + "</span>")
			} else if (e == l.d) {
				m++;
				f[c.dayText] = {};
			...
			validate: function(d) {
				...
				function(n, k) {
					if (l[k] !== void 0) {
						...
						if (c.invalid && k == "d") {
							var t = [];
							c.invalid.dates && a.each(c.invalid.dates,
							function(a, b) {
							...
							if (c.invalid.daysOfWeek) {
								...
								D;
								a.each(c.invalid.daysOfWeek,
								function(a, b) {
									for (D = b - y; D < F; D = D + 7) D >= 0 && t.push(D)
								})
							}
							c.invalid.daysOfMonth && a.each(c.invalid.daysOfMonth,
							function(a, b) {
								b = (b + "").split("/");
	...
	g.formatDate = function(g, j, p) {
		...
		k, p),
		u = function(a) {
			for (var c = 0; n + 1 < g.length && g.charAt(n + 1) == a;) c++,
			n++;
			return c
		},
		f = function(a, c, d) {
	...
	g.parseDate = function(g, j, p) {
		...
		l = -1,
		d = !1,
		v = function(a) { (a = A + 1 < g.length && g.charAt(A + 1) == a) && A++;
			return a
		},
		q = function(a) {
			v(a);
		...
		y = function(a, c, d) {
			a = v(a) ? d: c;
			for (c = 0; c < a.length; c++) if (j.substr(s, a[c].length).toLowerCase() == a[c].toLowerCase()) return s += a[c].length,
			c + 1;
			return 0
		},
		s = 0,
